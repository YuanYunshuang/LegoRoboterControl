...
Control::Control(ros::NodeHandle n) {
	...
	if (!_n.getParam("/nxt/turning_adaption", _turning_adaptation)) {
	...
	pubOdom = _n.advertise<geometry_msgs::PoseWithCovarianceStamped>("odom", 10);//Topic = "odom", msg_type=Position (x,y,z)+ quaternion orientation(x,y,z) +covariance, Wartschlange = 10
	pubEffort = _n.advertise<nxt_control::MotorCommand>("motor_command", 10);//Topic = "motor_command" msg_type = MotorCommand
	subCommand = _n.subscribe("cmd_vel", 1, &Control::getCommand, this);
	subSensors = _n.subscribe("sensor_data", 1000, &Control::getSensorData, this);
	sub_tracking = n.subscribe("tracking/pose", 1, &Control::robotPosCallback,this);
	pubNextcmd = _n.advertise<std_msgs::Bool>("nextcmd", 1000);
	pubUltrasonic=_n.advertise<sensor_msgs::Range>("ultrasonic",1000);
	_goal_reached = true;
}

void Control::robotPosCallback(const geometry_msgs::PoseStamped& msg) {
	if(fabs(msg.pose.position.x-camPose.x)>0.001||
			fabs(msg.pose.position.y-camPose.y)>0.001 ||
			fabs(msg.pose.position.x-camPose.theta)>0.001){
		camPose.x = msg.pose.position.x;
		camPose.y = msg.pose.position.y;
		camPose.theta = msg.pose.orientation.x;
		time_pose = ros::Time::now();
		imgPose = true;
		ROS_INFO("I'm at the position:[%f, %f, %f]", camPose.x, camPose.y,
				camPose.theta * 180 / M_PI);
	}
...
void Control::getCommand(const geometry_msgs::Twist& msg) {
	...
	//******************************************************** sonst: Rotation berechnen
		...
		double newTheta = fmod(_pose.th + msg.angular.z + 3 * M_PI,
				2 * M_PI)-M_PI;
...
nxt_control::MotorCommand Control::publishEffort(
	...
	ROS_INFO("vleft%f vRight%f", v_left, v_right);
	ROS_INFO("cmd l%f cmd r %f", cmd.l_effort, cmd.r_effort);
	return cmd;
}
...
void Control::updatePosition(const nxt_control::SensorData& msg ) {
	if(imgPose && ros::Time::now().sec-time_pose.sec<0.3){
		_pose.x = camPose.x;
		_pose.y = camPose.y;
		_pose.th = camPose.theta;
		imgPose = false;
	}else{
		ROS_INFO("ODOM POSE!!!!!");
		...
		double wheel_R = (2.0) * M_PI * _wheel_radius_r * msg.tickCountB
				/ _enc_per_turn_right;
		std::cout<<"wheel_L:" <<wheel_L<<", wheel_R:"<<wheel_R<<std::endl;


		if(fabs(0.5*(wheel_L+wheel_R)-x)<0.05&&x!=0)
				stopp = true;
		//turning angle
		double dtheta = (wheel_R - wheel_L) / _axis_length * _turning_adaptation;
		if(fabs(dtheta-z)<0.1&&z!=0)
			stopp = true;
		if (fabs(dtheta) < 0.005) {
			double wheel = (wheel_L + wheel_R) / 2.0;
			_pose.x += wheel * sin(_pose.th);
			_pose.y += wheel * cos(_pose.th);
			ROS_INFO("forward--------d_pose.x = %f, d_pose.y = %f",wheel * sin(_pose.th),wheel * cos(_pose.th));
		}
		else {
			double temp_x = _pose.y;
			double temp_y = _pose.x;
			double radius = wheel_L / dtheta;
			double cx = temp_x + (radius + _axis_length / 2.0) * sin(_pose.th);
			double cy = temp_y - (radius + _axis_length / 2.0) * cos(_pose.th);
			double newTheta = fmod(_pose.th - dtheta + 3 * M_PI, 2 * M_PI) - M_PI;
			_pose.y = cx - (radius + _axis_length / 2.0) * sin(newTheta);
			_pose.x = cy + (radius + _axis_length / 2.0) * cos(newTheta);
			_pose.th = newTheta;
		}
  }
}

geometry_msgs::PoseWithCovarianceStamped Control::publishOdom() {
	geometry_msgs::PoseWithCovarianceStamped odom;
	odom.header.frame_id="map";
	odom.header.stamp=ros::Time::now();
	odom.pose.pose.position.x = _pose.x;
	odom.pose.pose.position.y = _pose.y;
	odom.pose.pose.position.z = _pose.th;
	odom.pose.pose.orientation = tf::createQuaternionMsgFromYaw(_pose.th);
	return odom;
}
...
void Control::normSpeedTo(double& speed1, double& speed2, double maxspeed) {
	if (fabs(speed1) > maxspeed || fabs(speed2) > maxspeed) {
		while (fabs(speed1) > maxspeed || fabs(speed2) > maxspeed) {
			speed1 *= 0.9;
			speed2 *= 0.9;
			ROS_INFO("*09,%f ,%f", speed1, speed2);
		}
	} else {

		while (fabs(speed1) < maxspeed * 0.9 && fabs(speed2) < maxspeed * 0.9) {
			speed1 *= 1.1;
			speed2 *= 1.1;
			ROS_INFO("*1.1 %f,%f", speed1, speed2);
		}
	}
}
